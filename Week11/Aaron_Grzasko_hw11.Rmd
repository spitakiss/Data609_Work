...
![](https://raw.githubusercontent.com/spitakiss/Data609_Work/master/Week11/529_q6_img.PNG)  
...
First, find $f'(y)$ using the quotient rule:  
  
$$f'(y)=\frac{ay^{a-1}e^{by}-by^ae^{by}}{e^{2by}}$$  
  
$$=\frac{y^{a-1}(a-by)}{e^{by}}$$  
  
From the equation above, we determine two critical points where $f'(y)=0$:  
...
$$f''(a/b)=\frac{(\frac{a}{b})^{a-2}(a^2-a-2ab\frac{a}{b}+b^2\frac{a^2}{b^2})}{e^{b\frac{a}{b}}}$$  
...
On the other hand, if we confine $a$ and $b$ to positive real numbers, then $f''(y)$ is negative and we have a relative maximum.  
